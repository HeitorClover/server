...
function formatCPF(cpf) {
  ...
  // Verifica se tem 14 dígitos (CNPJ)
    ...
    const formatted = numbersOnly.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5');
    console.log(`✅ CNPJ formatado: ${numbersOnly} -> ${formatted}`);
    return formatted;
  }
  else {
    console.log(`⚠️ Número não tem formato reconhecido (CPF/CNPJ): ${numbersOnly} (${numbersOnly.length} dígitos)`);
    return cpf; // Retorna original se não tiver formato reconhecido
  }
}
  ...
  try {
    ...
    if (event.type === 'update_column_value') {
      ...
      if (columnTitle.toLowerCase() === 'cpf') {
        ...
        // Formatando o CPF/CNPJ (a função detecta automaticamente pelo tamanho)
        formattedValue = formatCPF(extractedValue);
        
        // Verificar se o valor já está formatado (para evitar loop)
        if (formattedValue !== extractedValue) {
          shouldUpdate = true;
          console.log(`🔧 Formatando: ${extractedValue} -> ${formattedValue}`);
        } else {
          console.log('ℹ️  Valor já está formatado, nenhuma ação necessária');
        }
...
// Rota de debug para testar manualmente CPF/CNPJ
